...
require_once dirname(__FILE__).'/custom/additional_blocks.inc.php';
...
if (isset($_REQUEST['favobj'])) {
		...
		elseif ($_REQUEST['favaction'] == 'refresh') {
			switch ($_REQUEST['favref']) {
				...
				case 'hat_dscvry':
					...
					break;
				case 'hat_ipmi':
					$ipmistat = make_ipmistat_summary();
					if(!empty($ipmistat)){
						$ipmistat->show();
					}
					break;
	...
	if ($_REQUEST['favobj'] == 'set_rf_rate') {
		if (str_in_array($_REQUEST['favref'], array('hat_syssum', 'hat_stszbx', 'hat_lastiss', 'hat_webovr', 'hat_dscvry', 'hat_hoststat', 'hat_ipmi', 'hat_vsphere', 'hat_aws'))) {
			$widgetName = substr($_REQUEST['favref'], 4);
...
$refresh_tab = array(
	...
	array('id' => 'hat_ipmi','frequency'  => CProfile::get('web.dashboard.widget.ipmi.rf_rate', 60)),
	array('id' => 'hat_vsphere','frequency'  => CProfile::get('web.dashboard.widget.vsphere.rf_rate', 60)),
	array('id' => 'hat_aws','frequency'  => CProfile::get('web.dashboard.widget.aws.rf_rate', 60))
);
...
if(USER_TYPE_SUPER_ADMIN == CWebUser::$data['type']){
	$refresh_menu = get_icon('menu', array('menu' => 'hat_ipmi'));
	$ipmistat_summary = make_ipmistat_summary(true);
	if(!empty($ipmistat_summary)){
		$ipmistat = new CUIWidget('hat_ipmi', $ipmistat_summary ,CProfile::get('web.dashboard.widget.ipmi.state',1));
		$ipmistat->setHeader(_('Physical machine status'),array($refresh_menu));
		$ipmistat->setFooter(new CDiv(SPACE, 'textwhite', 'hat_ipmistat_footer'));
		$col = CProfile::get('web.dashboard.widget.ipmi.col', '1');
		$row = CProfile::get('web.dashboard.widget.ipmi.row', '5');
		if (!isset($columns[$col][$row])) {
			$columns[$col][$row] = $ipmistat;
		}
		else {
			$columns[$col][] = $ipmistat;
		}
	}
}
...
// vSphere status
if(USER_TYPE_SUPER_ADMIN == CWebUser::$data['type']){
	$refresh_menu = get_icon('menu', array('menu' => 'hat_vsphere'));
	$vspherestat_summary = make_vspherestat_summary(true);
	if(!empty($vspherestat_summary)){
		$vspherestat = new CUIWidget('hat_vsphere', $vspherestat_summary ,CProfile::get('web.dashboard.widget.vsphere.state',1));
		$vspherestat->setHeader(_('vSphere status'),array($refresh_menu));
		$vspherestat->setFooter(new CDiv(SPACE, 'textwhite', 'hat_vspherestat_footer'));
		$col = CProfile::get('web.dashboard.widget.vsphere.col', '1');
		$row = CProfile::get('web.dashboard.widget.vsphere.row', '3');
		if (!isset($columns[$col][$row])) {
			$columns[$col][$row] = $vspherestat;
		}
		else {
			$columns[$col][] = $vspherestat;
		}
	}
}
...
// AWS status
if(USER_TYPE_SUPER_ADMIN == CWebUser::$data['type']){
	$refresh_menu = get_icon('menu', array('menu' => 'hat_aws'));
	$awsstat_summary = make_awsstat_summary(true);
	if(!empty($awsstat_summary)){
		$awsstat = new CUIWidget('hat_aws', $awsstat_summary, CProfile::get('web.dashboard.widget.aws.state', 1));
		$awsstat->setHeader(_('Amazon Web Services status'),array($refresh_menu));
		$awsstat->setFooter(new CDiv(SPACE, 'textwhite', 'hat_awsstat_footer'));
		$col = CProfile::get('web.dashboard.widget.aws.col', '1');
		$row = CProfile::get('web.dashboard.widget.aws.row', '4');
		if (!isset($columns[$col][$row])) {
			$columns[$col][$row] = $awsstat;
		}
		else {
			$columns[$col][] = $awsstat;
		}
...
$row = CProfile::get('web.dashboard.widget.lastiss.row', '6');
if (!isset($columns[$col][$row])) {
...
$row = CProfile::get('web.dashboard.widget.webovr.row', '7');
if (!isset($columns[$col][$row])) {
...
<?php
require_once dirname(__FILE__).'/custom/monitoring.dashboard.js.php';
require_once dirname(__FILE__).'/include/page_footer.php';
